...
MAX_IMPORT_NUDGE = 1       # missing imports in added lines (ported from ProjectNobi v19)
MAX_ROUTE_NUDGE = 1        # new page/component without route file (ported from ProjectNobi v19)
MAX_TOTAL_REFINEMENT_TURNS = 4  # raised to 4 to admit deletion+import+route+criteria as needed;
...
def _profile_repo(repo: Path, tracked: Optional[List[str]] = None) -> Dict[str, bool]:
    """Lightweight repo profile used to tailor the initial user-prompt hint
    block. Detects test framework, presence of migrations dir, presence of
    a router/urls file. Cheap: only path checks, no file reads.
    """
    if tracked is None:
        try:
            ...
            tracked = []
    files_lower = {p.lower() for p in tracked}
    has_pytest_config = any(
        n in files_lower for n in ("pytest.ini", "pyproject.toml", "setup.cfg", "tox.ini")
    )
    has_jest_config = any(
        n in files_lower for n in ("jest.config.js", "jest.config.ts", "jest.config.mjs")
    )
    has_go_mod = "go.mod" in files_lower
    has_cargo = "cargo.toml" in files_lower
    has_migrations = any("migration" in f for f in files_lower)
    has_routes_file = any(
        any(h in f for h in ("/routes/", "router.", "urls.py", "/router/", "routes.ts"))
        for f in files_lower
    )
    return {
        "pytest": has_pytest_config,
        "jest": has_jest_config,
        "go": has_go_mod,
        "rust": has_cargo,
        "migrations": has_migrations,
        "routes": has_routes_file,
    }


def _build_profile_hint(profile: Dict[str, bool]) -> str:
    """Turn the profile flags into a compact 'how to verify in THIS repo' block.
    Returns "" when no useful hint can be given (silent no-op)."""
    bullets: List[str] = []
    if profile.get("pytest"):
        bullets.append("- Verify with a focused pytest invocation: `pytest path/to/test_X.py -x -q`")
    if profile.get("jest"):
        bullets.append("- Verify with: `npx jest path/to/file.test.ts -t 'pattern'`")
    if profile.get("go"):
        bullets.append("- Verify with: `go test ./pkg/foo -run TestName`")
    if profile.get("rust"):
        bullets.append("- Verify with: `cargo test specific_test_name`")
    if profile.get("migrations"):
        bullets.append("- Repo has a migrations directory; model/schema changes likely need a migration file in the same diff.")
    if profile.get("routes"):
        bullets.append("- Repo has explicit routes/urls file; new pages or endpoints need the route registered in the same diff.")
    if not bullets:
        return ""
    return "REPO PROFILE (verification + wiring shortcuts for this repo):\n" + "\n".join(bullets) + "\n\n"
...
def _broad_grep_fallback(repo: Path, issue_text: str, tracked: set) -> List[str]:
    ...
    def _grep_one(term: str) -> List[str]:
        try:
            proc = subprocess.run(
                ["git", "grep", "-l", "-i", "-F", "--", term],
                cwd=repo_str,
    ...
    hits: Dict[str, int] = {}
    with ThreadPoolExecutor(max_workers=min(6, len(terms))) as ex:
        for matched in ex.map(_grep_one, terms):
...
# Import + route wiring gates. Two high-precision detectors for common
# multi-file failures: missing imports and missing route registration.

_ROUTE_SIGNAL_WORDS: Tuple[str, ...] = (
    "route", "navigation", "nav", "page", "url", "link",
    "router", "redirect", "path", "endpoint", "menu item",
    "req.params", ":param", "controller", "handler",
)
_ROUTE_FILE_HINTS: Tuple[str, ...] = (
    "router", "routes", "navigation", "nav", "menu",
    "sidebar", "app.py", "urls.py", "index.ts",
)
_PAGE_FILE_HINTS: Tuple[str, ...] = ("page", "view", "component")


def _detect_missing_route_wiring(task_text: str, patch: str) -> List[str]:
    """When a new page/component is added but no router/menu file touched."""
    task_lower = task_text.lower()
    if sum(1 for s in _ROUTE_SIGNAL_WORDS if s in task_lower) < 2:
        return []
    touched = {m.group(1).lower()
               for m in re.finditer(r'^(?:\+\+\+|---) [ab]/(.+)$', patch, re.M)}
    if any(h in f for f in touched for h in _PAGE_FILE_HINTS) and \
       not any(h in f for f in touched for h in _ROUTE_FILE_HINTS):
        return ["new page/component added but no route file touched — wire URL + nav"]
    return []


def build_route_wiring_prompt(gaps: List[str]) -> str:
...
def _detect_missing_imports(patch: str) -> List[str]:
    """Heuristic: CamelCase tokens in added Python lines that don't appear
    in any added `from`/`import` line. Fires only when 3+ such tokens look
    suspect, keeping false-positive rate low.
    """
    added_text = "\n".join(
    ...
    if not added_text:
        return []
    used = set(re.findall(r"\b([A-Z][a-zA-Z]+)\b(?!\s*[:(=])", added_text))
    import_lines = re.findall(r"^(?:from|import)\s+\S+", added_text, re.M)
    imported = set(re.findall(r"\b([A-Z][a-zA-Z]+)\b", " ".join(import_lines)))
    suspect = used - imported
    if len(suspect) > 3:
        return [f"Possible missing imports for: {', '.join(sorted(suspect)[:5])}"]
    return []


def build_import_nudge_prompt(missing: List[str]) -> str:
    ...
    return (
        ...
        "<final>already imported</final>. Otherwise add the minimal import "
        "lines and end with <final>summary</final>."
    )
...
def _symbol_grep_hits(
    ...
    Per-symbol greps run in parallel so wall-clock is O(max grep), not O(N*timeout).
    """
    ...
    def _grep_one(symbol: str) -> List[str]:
        try:
            proc = subprocess.run(
                ["git", "grep", "-l", "-F", "--", symbol],
                cwd=repo_str,
    ...
    with ThreadPoolExecutor(max_workers=min(8, len(symbols))) as ex:
        for matched in ex.map(_grep_one, symbols):
            for relative_path in matched:
...
def build_attempt2_bootstrap(result1: Dict[str, Any], n_lines: int) -> str:
    """Attempt-2 prefix. Summarises why attempt 1 failed AND surfaces files
    already edited + recent greps so attempt 2 actively diverges instead of
    only being told to try something different."""
    steps = result1.get("steps", 0)
    logs_text = result1.get("logs", "") or ""
    patch_text = result1.get("patch", "") or ""
    ...
    files_tried = _patch_changed_files(patch_text)
    files_tried_block = ""
    if files_tried:
        files_tried_block = (
            "Files attempt 1 edited (consider whether the fix actually belongs elsewhere):\n"
            + "".join(f"  - {p}\n" for p in files_tried[:5])
        )

    grep_history = re.findall(r"git grep[^\n]+", logs_text)
    recent_greps = grep_history[-3:] if grep_history else []
    grep_block = ""
    if recent_greps:
        grep_block = (
            "Recent searches attempt 1 ran (these didn't unlock the fix — try different terms):\n"
            + "".join(f"  - {g[:120]}\n" for g in recent_greps)
        )

    return (
        f"⚠ RETRY ATTEMPT: A prior attempt {reason_str} ({steps} steps). "
        "Do NOT repeat the same approach.\n"
        f"{files_tried_block}{grep_block}"
        "Re-read the issue first. Pick a different target file, run a NEW grep "
        "targeting issue terms not yet tried, then make the smallest correct "
        "edit and end with <final>.\n\n"
    )
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, test_fix_turns_used, coverage_nudges_used, criteria_nudges_used, hail_mary_turns_used, total_refinement_turns_used, must_edit_after_gap, must_edit_patch, gap_edit_nudges_used, deletion_nudges_used, import_nudge_used, route_nudge_used
        ...
        # Import completeness: CamelCase tokens used but not imported.
        if import_nudge_used < MAX_IMPORT_NUDGE and total_refinement_turns_used < MAX_TOTAL_REFINEMENT_TURNS:
            ...
            if _missing_imps:
                ...
                queue_refinement_turn(
                    assistant_text,
                    build_import_nudge_prompt(_missing_imps),
                    "IMPORT_NUDGE_QUEUED",
                )
                return True

        # Route wiring: new page/component added without route or nav file touched.
        if route_nudge_used < MAX_ROUTE_NUDGE and total_refinement_turns_used < MAX_TOTAL_REFINEMENT_TURNS:
            ...
            if _route_gaps:
                ...
                queue_refinement_turn(
                    assistant_text,
                    build_route_wiring_prompt(_route_gaps),
                    "ROUTE_NUDGE_QUEUED",
                )
                return True
    ...
    try:
        ...
        _profile_hint = _build_profile_hint(_profile_repo(repo))

        _initial_user_content = (
            (prior_attempt_summary if prior_attempt_summary else "")
            + (_profile_hint if _profile_hint else "")
        ...
        for step in range(1, max_steps + 1):
            ...
            # Iterative rescue: re-run broad-grep mid-solve if stuck without a patch.
            if (not iterative_rescue_fired
                    and step >= 4
                    and not get_patch(repo).strip()):
                iterative_rescue_fired = True
                _tracked = set(_tracked_files(repo))
                _resc = _broad_grep_fallback(repo, issue, _tracked)
                if _resc:
                    logs.append("ITERATIVE_RESCUE_FIRED:\n  " + ", ".join(_resc))
                    messages.append({
                        "role": "user",
                        "content": (
                            "STUCK CHECK: 4+ steps without a patch. Broad-grep matches "
                            "for the issue's natural-language terms:\n"
                            + "".join(f"  - {p}\n" for p in _resc)
                            + "Open the first one (cat -n), then edit. Stop searching."
                        ),
                    })
