...
_SCOPE_NARROW_PHRASES = (
    "only modify", "just add", "simply add", "small change",
    "one-line", "minor fix", "single-line", "trivial",
)
_SCOPE_WIDE_PHRASES = (
    "across multiple", "throughout the codebase", "in every",
    "and the test", "and update the test", "and wire", "and update",
    "import path", "migration", "schema", "serializer",
    "and ensure", "also add", "also update",
)


def _estimate_scope_size(issue: str, mentioned_count: int, bullets: int) -> str:
    """Estimate the SCOPE of the issue based on text signals.

    Returns one of 'narrow', 'medium', 'wide'. Used by build_preloaded_context
    to cap the number of preloaded files — narrow scope = fewer files (avoid
    tempting model to over-rewrite), wide scope = more files (cover all
    integration points).

    Signals:
      narrow: explicit phrases like "only modify X", "small change", one
              mentioned file with no integration words.
      wide:   multiple files mentioned, "across multiple", "and update test",
              integration phrases (migration, serializer, route+handler).
      medium: default fallback.
    """
    issue_lower = issue.lower()
    # Hard narrow signal
    for p in _SCOPE_NARROW_PHRASES:
        if p in issue_lower:
            return "narrow"

    indicator_count = 0
    indicator_count += min(4, mentioned_count) * 2  # explicit path mentions weigh heavily
    indicator_count += min(3, len(_BACKTICK_IDENT_RE.findall(issue)))
    for p in _SCOPE_WIDE_PHRASES:
        if p in issue_lower:
            indicator_count += 1
    if bullets >= 3:
        indicator_count += min(4, bullets // 2)

    if indicator_count >= 7:
        return "wide"
    if indicator_count >= 3:
        return "medium"
    # If only one file mentioned and no other strong signal: narrow
    if mentioned_count <= 1 and indicator_count <= 1:
        return "narrow"
    return "medium"
...
def build_preloaded_context(repo: Path, issue: str) -> Tuple[str, List[str]]:
    ...
    # Scope-aware file cap: narrow issues get fewer preloaded files (avoids
    # tempting the model to rewrite unrelated code); wide issues get the full
    # preload set so multi-file integration points are visible. Empirically,
    # current king loses ~22/25 challengers on tasks where wide preload tempts
    # over-editing of a single file, and ~17/22 where narrow preload misses
    # multi-file integration points.
    mentioned_count = sum(1 for p in _extract_issue_path_mentions(issue)
                          if p.strip("./") in tracked_set)
    bullets = sum(1 for line in issue.splitlines()
                  if line.strip().startswith(("-", "*", "1.", "2.", "3.", "4.")))
    scope = _estimate_scope_size(issue, mentioned_count, bullets)
    if scope == "narrow":
        scope_file_cap = min(MAX_PRELOADED_FILES, 4)
    elif scope == "wide":
        scope_file_cap = MAX_PRELOADED_FILES
    else:  # medium
        scope_file_cap = min(MAX_PRELOADED_FILES, 10)

    parts: List[str] = []
    included: List[str] = []
    used = 0
    n_files = min(len(files), scope_file_cap)
    ...
    for file_idx, relative_path in enumerate(files[:scope_file_cap]):
